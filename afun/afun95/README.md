...
In the current folder of this current repository there two subfolder that we will examine:
- deployment
- functionapp-demo 

Let's start by the **deployment** subfolder. In this one you will find two Azure Resource Manager (ARM) template. The ARM template use a formatting really close to JSon, and it's use to create the Azure instances for our solution. Not the binaries, only the infrastructure.

Open the file `deployAzure-afun95-demo3.json`, this is the file we will use for this demo. There is four major sections to an ARM template: parameters, variables, resources, and outputs. 
...
Parameters and Variables are very useful to create a more dynamic content. Have a look to the few parameters and variables name, we will get back to them later.

### The Resources

There it is the core of the ARM template the list of resources to create. To create an Azure Function we need a few resources: a functionapp (aka.Web/sites), an storageAccounts to save the code, and a Service Plan or "serverfarms" that describe the physical resources that will be use and some billing options. Try to find those in elements in the template, and examine the properties. 
...
To deploy the Function we will send the ARM template define in the deployment folder to the Azure portal `#create` feature. This is done by appending the absolute path of our template to the URL of the portal. The result look like this.
...
```
A frequent way to ease the process is to put that composed URL in a anchor. Click the Deploy to Azure anchor (or button) to start the deployment.
...
This will open the Azure portal with a form that we saw previously. This time you should recognize the fields of the form matching the parameters of our ARM template...  If you *Forked* the GitHub Repo your can change the URL to see your changes.
...
To get more tutorial, code example, and documentation visit the [Azure Resource Manager](https://docs.microsoft.com/en-us/azure/azure-resource-manager/resource-group-overview?WT.mc_id=msignitethetour2019-github-afun95).


## Teardown Instructions

Navigate to the Azure Portal and delete the Azure Resource Group (ex: AFUN95demo) you created earlier. Deleting the Resource Group will delete all resource within. Once the command is started you don't need to way, the command is been executed in Azure. \
